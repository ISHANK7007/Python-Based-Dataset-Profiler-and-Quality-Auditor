...
# Ensure current directory is treated as root for module imports
...
from cli.cli_rule_exit_handler import apply_exit_logic  # newly added
from cli.cli_validation_command import validate_rules_from_config  # newly added
...
def main():
    ...
    # Optional: rule config-based validation
    parser.add_argument("--rule-config", help="Path to standalone expectation config file (.yaml)")
    ...
    if args.audit_policy:
        ...
        try:
            ...
            apply_exit_logic(audit_results, policy)

        except Exception as e:
            print(f"[ERROR] Audit failed: {e}")
            sys.exit(2)

    # === Optional: Expectation-Based Validation ===
    if args.rule_config:
        try:
            print(f"\n[INFO] Validating against rule config → {args.rule_config}")
            exit_code = validate_rules_from_config(profile, args.rule_config)
            sys.exit(exit_code)
        except Exception as e:
            print(f"[ERROR] Validation from config failed: {e}")
            sys.exit(3)
    ...
    if args.output_report:
        ...
        report = renderer.render_report(
            audit_results or profile.to_dict(),
            context={"dry_run": audit_results.get("dry_run", False)}
        )
